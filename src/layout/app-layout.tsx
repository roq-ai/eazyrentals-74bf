...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['property-owner', 'property-manager'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<LandlordInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/landlords` : null),
    () => getLandlords({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Landlord',
      path: isTenantUser ? `/landlords/view/${data?.[0]?.id}` : '/landlords',
      entity: 'landlord',
      service: AccessServiceEnum.PROJECT,
      icon: FiUserCheck,
    },
    { name: 'Lease', path: '/leases', entity: 'lease', service: AccessServiceEnum.PROJECT, icon: FiFileText },
